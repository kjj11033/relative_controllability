function  [dat1, dat2, traj, infStates] = tapas_controllability_vol_bias1_LR_simulation(r)
% ============================================================
p=r.p_prc.p;
...
%         mu_other(1,1)
       %%
        ...
        omega = om(2);%-(ctr_muhat(k,1)); % CTR-multiplied
        % Precision of prediction
        pihat(k,2) = 1/(1/pi(k-1,2) +exp(ka(2) *mu(k-1,3) + omega));%om(2)));
        ...
        % Weighting factor
        v(k,l)   = t(k) *th;
        v(k,l-1) = t(k) *exp(ka(l-1) *mu(k-1,l) + omega);%om(l-1));
...
%         mu(k,:) = muhat(k,:) + [alpha_self alpha_other].*tapas_sgm([ctr_muhat(k) 1-ctr_muhat(k)],1).*rew_PE(k,:);
%         u_other(k)
        mu_other(k) = muhat_other(k) + alpha_other.*rpe.*(1-ctr_muhat(k,1));
