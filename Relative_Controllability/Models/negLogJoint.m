...
% predictions on.
try
    [dummy, infStates] = prc_fun(r, ptrans_prc, 'trans'); % prc_fun 이 HGF 인 경우
    % inf state 는 muhat, sahat, mu, sa 를 return 한다. (sa, sahat, mu, muhat, 1~trial끝까지 전부)
    % trans 는 transformed param 을 사용할 것인지에 대한 것
catch err
    negLogJoint = realmax;
    negLogLl = realmax;
...
trialLogLls = obs_fun(r, infStates, ptrans_obs); %obs_fun 은 unit square sigmoid 인데, 이를 이용
