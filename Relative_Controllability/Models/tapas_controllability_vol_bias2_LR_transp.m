function [pvec, pstruct] = tapas_controllability_vol_bias2_LR_transp(r, ptrans)
% --------------------------------------------------------------------------------------------------
...
pvec(5*l)         = tapas_sgm(ptrans(5*l),1);                     % om
pstruct.theta_rp  = pvec(5*l);
pvec(5*l+1)       = tapas_sgm(ptrans(5*l+1),1);                  % sa_0
pstruct.alpha_other     = pvec(5*l+1);
pvec(5*l+2)       = exp(ptrans(5*l+2));                  % sa_0
pstruct.selc      = pvec(5*l+2);
pvec(5*l+3)       = exp(ptrans(5*l+3));                  % sa_0
pstruct.selw      = pvec(5*l+3);
