function [dat1, dat2, traj, infStates] = tapas_controllability_lrbias2_simulation(r)
% ============================================================
p = r.p_prc.p;
...
for k = 2:1:n
    if not(ismember(k-1, r.ign))
        ctr_muhat(k,:) = 0.5;%ctr_mu(k-1,:)+theta*(1/2-ctr_mu(k-1,:)); % considers drift         
        muhat(k,:) = mu(k-1,:);       
        % ============================================================
        ...
        if feedback(k) == 1            
            ctr_mu(k) = 0.5;%(lik_mine*ctr_muhat(k))/(lik_mine*ctr_muhat(k)+lik_other*(1-ctr_muhat(k)));
        elseif feedback(k) == -1
            ctr_mu(k) = 0.5;%(lik_mine*ctr_muhat(k))/(lik_mine*ctr_muhat(k)+lik_other*(1-ctr_muhat(k)));
        end
    else
