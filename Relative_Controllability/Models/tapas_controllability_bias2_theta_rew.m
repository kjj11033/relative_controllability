function [traj, infStates] = tapas_controllability_bias2_theta_rew(r, p, varargin)
...
%         mu(k,:) = muhat(k,:) + [alpha_self alpha_other].*tapas_sgm([ctr_muhat(k) 1-ctr_muhat(k)],1).*rew_PE(k,:);
        mu(k,:) = muhat(k,:) + [alpha_self alpha_other].*rew_PE(k,:); %*(1-ctr_muhat(k))
        % drift increases as ctr decreases
