function [traj, infStates] = tapas_controllability_lrbias2(r, p, varargin)
...
if ~isempty(varargin) && strcmp(varargin{1},'trans');
    p = tapas_controllability_bias2_theta_rew_transp(r, p);
end
    ...
    if not(ismember(k-1, r.ign))
       ...
       %% ==========  Update controllability based on posterior=========
        ...
        if feedback(k) == 1            
           ...
           mu(k,:) = muhat(k,:) + [alpha_self_rew alpha_other].*rew_PE(k,:); %*(1-ctr_muhat(k))           
        elseif feedback(k) == -1
           rew_PE(k,1) = (1-u_self(k))-muhat(k,1);
           rew_PE(k,2) = (1-u_other(k))-muhat(k,2);
           mu(k,:) = muhat(k,:) + [alpha_self_pun alpha_other].*rew_PE(k,:); %*(1-ctr_muhat(k))           
        end
